# app/schemas/local/local_external_import.py
...
class ExternalLocalImport(BaseModel):
    """
    HTTP-level schema used to receive import filters for the external
    LocalInfo API.

    It mirrors the search filters, but does not support `skip` and uses
    `limit` as an upper bound for how many venues will be imported.
    """
